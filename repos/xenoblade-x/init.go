...
func init() {
	...
	s3SecureEnv := os.Getenv("PN_XCX_CONFIG_S3_SECURE")

	postgresURI := os.Getenv("PN_XCX_POSTGRES_URI")
	kerberosPassword := os.Getenv("PN_XCX_KERBEROS_PASSWORD")
	...
	if strings.TrimSpace(postgresURI) == "" {
		globals.Logger.Error("PN_XCX_POSTGRES_URI environment variable not set")
		os.Exit(0)
	}

	if strings.TrimSpace(kerberosPassword) == "" {
		globals.Logger.Warningf("PN_XCX_KERBEROS_PASSWORD environment variable not set. Using default password: %q", globals.KerberosPassword)
	} else {
	...
	if err != nil {
		globals.Logger.Warningf("PN_XCX_CONFIG_S3_SECURE environment variable not set. Using default value: %t", true)
		s3Secure = true
	}
